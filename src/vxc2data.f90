...
INTEGER, INTENT(OUT) :: numfun,       &      ! number of functions for calculating external sources
                        ...
                        size_PHYS_C,  &      ! number of domains with diffusion properties
...
REAL(8),          INTENT(OUT) :: BND(3,2)   ! values of Dirichlet boundary conditions on 6 faces
!-------------------!
! working variables !
!-------------------!
REAL(8) delta0
...
INTEGER num_nodx, num_nody, num_nodz
...
! namelist /WRITEdata/  &
    ...
    ! numfun, numMech, num_nodx, num_nody,  num_nodz, num_nodV, num_nod0,     &
...
DO WHILE ( 1==1)
    ...
    IF (k0>0 ) THEN ! 
        ...
        DO i=2,neww
            IF (index( words(i)(1:1),'D') /=0 ) THEN
                ...
                ENDDO
                DO j=i+2, neww
                    IF ( index( words(j)(1:2),'U0')/=0) THEN 
                        size_PHYS_U0 = size_PHYS_U0 + 1
                        ! PRINT*,'size_PHYS_U0=',size_PHYS_U0
                    ENDIF
                ENDDO
                DO j=i+2, neww
                    IF ( index( words(j),'UBND')/=0) THEN 
                        size_PHYS_Ubnd = size_PHYS_Ubnd + 1
                    ENDIF
                ENDDO
                IF ( index( words(i+2),'SRC') /=0) THEN 
                    DO j=i+2, neww
                        ...
                        ELSEIF ( index( words(j),'SRCZ') /=0) THEN
                            numfun = numfun + 1
                        ELSEIF ( index( words(j),'SRC0') /=0) THEN
                            numfun = numfun + 1
                        ...
                        ELSEIF ( index( words(j),'VSZ') /=0) THEN
                            numMech = numMech + 1
                        ELSE
                            !STOP ' err no define exitation X Y or Z or 0'
                        ENDIF
                    ENDDO
                ENDIF    
            ENDIF
            ...
            IF (trim(words(i)) == 'SOLVER') THEN 
                DO j=i+1,neww-1
                    ...
                    ELSEIF ( index(words(j),'FILES') /=0) THEN
                        files = trim(words(j+1))
                        WRITE(*,'( a,a,$ )') ' files=',trim(files)
    ...
    IF (m1>=1 .and. m==0) THEN
        ...
        READ(1,'(a)',end=91,iostat=ios) ch_e ! пропуск строки   <Data>
        IF ( uncompress ) THEN
        ! START UNCOMPRESS
...
IF (  size_PHYS_U0 /=0  )  THEN
    PRINT*,'numbers domains  with U0 = ',size_PHYS_U0
    ALLOCATE ( PHYS_U0(size_PHYS_U0) )
ENDIF

IF (  size_PHYS_Ubnd /=0 )  THEN
    PRINT*,'numbers domains  with Ubnd = ', size_PHYS_Ubnd
    ALLOCATE ( PHYS_Ubnd(size_PHYS_Ubnd) )
ENDIF
    ...
    IF ( k0 > 0 ) THEN 
        ...
        DO i=2,neww
            IF ( words(i)(1:1)=='D' .and. kp <= nsub ) THEN  ! Key word D out environ
                ...
                DO j=i+2, neww-1
                    IF ( words(j)(1:2) ==  'U0') THEN 
                        size_PHYS_U0 = size_PHYS_U0 + 1
                        PHYS_U0(size_PHYS_U0)%valdom = evaluate(words(j + 1))
                        PHYS_U0(size_PHYS_U0)%numdom = kp
                        PRINT '( "nsub= ",i3,  " U0= ",e10.3,  " typPHYS= ", a,   " namPHYS= ", a )', &
                                  kp ,       PHYS_U0(size_PHYS_U0)%valdom,  trim(typPHYS(kp)),   trim(namePHYS(kp))
                    ENDIF
                ENDDO
        
                DO j=i+2, neww-1
                    IF ( words(j)(1:4) ==  'UBND') THEN 
                        size_PHYS_Ubnd = size_PHYS_Ubnd + 1
                        PHYS_Ubnd(size_PHYS_Ubnd)%valdom = evaluate(words(j + 1))
                        PHYS_Ubnd(size_PHYS_Ubnd)%numdom = kp
                        PRINT '( "nsub= ",i3,  " Ubnd= ",e10.3,  " typPHYS= ", a,   " namPHYS= ", a  )', &
                                  kp ,       PHYS_Ubnd(size_PHYS_Ubnd)%valdom,  trim(typPHYS(kp)),   trim(namePHYS(kp))
                    ENDIF
                ENDDO
...
ALLOCATE (geoPHYS(sdx,sdy,sdz), geoUbnd(sdx,sdy,sdz),  source=0_1  )
...
! the number of node numbers for each np from the list PHYS_C(i)%numdom
    DO i=1,size_PHYS_C
        np = PHYS_C(i)%numdom
        n = count(v == np)
        PHYS_C(i)%siznod = n
...
! the number of node numbers for each np from the list PHYS_U0(i)%numdom 
IF (size_PHYS_U0 /= 0) THEN
    DO i=1,size_PHYS_U0
        np =  PHYS_U0(i)%numdom 
        n = count(v == np)
        PHYS_U0(i)%siznod = n
        ALLOCATE (PHYS_U0(i)%nod(n), source=0   )
        k=0
        DO j=1,Cells
            IF (v(j) == np) THEN
                k=k+1
...
! the number of node numbers for each np from the list  PHYS_Ubnd(i)%numdom   
IF (size_PHYS_Ubnd /= 0) THEN
    DO i=1,size_PHYS_Ubnd
        np =  PHYS_Ubnd(i)%numdom
        n = count(v == np)
        PHYS_Ubnd(i)%siznod = n
        ALLOCATE (PHYS_Ubnd(i)%nod(n), source=0   )
        k=0
        DO j=1,Cells
            IF (v(j) == np) THEN
                k=k+1
        ...
        ENDDO
    ENDDO
    geoUbnd = reshape(v_e, shape =(/sdx,sdy,sdz/) ,  order = (/ 1, 2, 3/) )
    v_e = 0
ENDIF

DEALLOCATE (v)

IF (numfun ==0 .and. size_PHYS_U0 ==0 .and. size_PHYS_Ubnd == 0 .and. (count(BND == 0.d0) ==6) ) THEN
    STOP 'not source, or U0, or BND'
ELSEIF (numfun ==0 .and.  (size_PHYS_U0 /=0 .or. size_PHYS_Ubnd /=0 .or.  count(BND == 0.d0) /=6 ) ) THEN
    num_nod0 = 1  ! is pure boundary problem for scalar field
ENDIF
...
IF (numfun /=0 ) THEN
    ...
    num_nodx = 0; num_nody = 0; num_nodz =0 ; num_nod0 =0 
    DO m=1,numfun                                          ! ALLOCATION OF MEMORY
        ...
        IF ( fun_nod(m)%numnod_Fz /= 0) THEN 
            ...
            num_nodz = num_nodz + n
        ENDIF
        IF ( fun_nod(m)%numnod_F0 /= 0) THEN 
            n = fun_nod(m)%numnod_F0 
            ALLOCATE(fun_nod(m)%nods_F0(n), source=0) 
            num_nod0 = num_nod0 + n
        ENDIF
    ENDDO
    ...
    ! WRITE(*, '(  *(a,i6) )') ' num_nod0=', num_nod0, ' num_nodV=', num_nodV

    IF (num_nodV /=0 ) fun_nod(:)%numnod_Fx=0; fun_nod(:)%numnod_Fy=0; fun_nod(:)%numnod_Fz=0; 
    IF (num_nod0 /=0 ) fun_nod(:)%numnod_F0=0;
    
    DO m=1,num_nodV + num_nod0
        k=sp_F%jm  
        n=sp_F%im  
        ch = sp_F%ch   
        IF       (trim(ch) == 'X') THEN 
        ...
        ELSEIF (trim(ch) == 'Y') THEN
            ...
            fun_nod(k)%nods_Fy(k0) = n 
        ELSEIF (trim(ch) == 'Z') THEN
            fun_nod(k)%numnod_Fz = fun_nod(k)%numnod_Fz + 1
            k0 = fun_nod(k)%numnod_Fz
            fun_nod(k)%nods_Fz(k0) = n 
        ELSEIF (trim(ch) == '0') THEN
            fun_nod(k)%numnod_F0 = fun_nod(k)%numnod_F0 + 1
            k0 = fun_nod(k)%numnod_F0
            fun_nod(k)%nods_F0(k0) = n 
        ELSE
        ENDIF
        sp_F => sp_F%prec
    ENDDO
        ...
        IF ( j+1 + n+1 <= neww) THEN !   number for Vsx or Vsy Vsz
            ...
            ELSEIF ( index( words(j+1 + n ),'VSY') /=0 ) THEN
                ...
                SELECT CASE (words(j+1 + n+1)(1:1) )        ! analysis of the character after the = sign
                    ...
                    CASE DEFAULT                                 ! the rest are either a number or quotes
                        fun_nod(numfun)%num_Vmech(2) = 0         ! зwriting the number of the mechanical function = 0
